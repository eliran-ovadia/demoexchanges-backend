...
def getPortfolio(db: Session = check_db, current_user: schemas.TokenData = check_auth) -> dict:
...
def getHistory(db: Session = check_db, current_user: schemas.TokenData = check_auth) -> List[schemas.History]:
