...
from src.exchange.schemas.schemas import TokenData, CreateUser, MessageResponse
...
async def create_user(request: Request, body: CreateUser, db: AsyncSession = check_db) -> MessageResponse:
    return await user_repo.create_user(body, db)


@router.patch('/portfolio/reset', response_model=MessageResponse, status_code=status.HTTP_200_OK)
@limiter.limit("5/minute")
...
@router.delete('/users', response_model=MessageResponse, status_code=status.HTTP_200_OK)
